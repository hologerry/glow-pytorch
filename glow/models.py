...
class FontGlow(nn.Module):
    ...
    def prior(self, y=None):
        ...
        if self.hparams.Glow.y_condition:
            ...
            yp = self.project_ycond(y).view(B, C, 1, 1)
            h += yp
        return thops.split_feature(h, "split")
    ...
    def forward(self, x=None, c=None, y=None, z=None,
                eps_std=None, reverse=False):
        if not reverse:
            return self.normal_flow(x, c, y)
        else:
            return self.reverse_flow(z, c, y, eps_std)

    def normal_flow(self, x, c, y):
        pixels = thops.pixels(x)
        z = x + torch.normal(mean=torch.zeros_like(x),
                             std=torch.ones_like(x) * (1. / 256.))
        logdet = torch.zeros_like(x[:, 0, 0, 0])
        ...
        if self.hparams.Glow.y_condition:
            c_logits = self.project_c_class(z.mean(2).mean(2))
            y_logits = self.project_class(z.mean(2).mean(2))
        else:
            c_logits = None
            y_logits = None
        ...
        # return
        nll = (-objective) / float(np.log(2.) * pixels)
        return z, nll, c_logits, y_logits

    def reverse_flow(self, z, c, y, eps_std):
        with torch.no_grad():
            mean, logs = self.prior(y)
            if z is None:
                z = modules.GaussianDiag.sample(mean, logs, eps_std)
    ...
    @staticmethod
    def loss_l1(y_logits, y):
        if y_logits is None:
            return 0
        else:
            return FontGlow.L1(y_logits, y)
    ...
    def loss_multi_classes(y_logits, y_onehot):
        if y_logits is None:
            ...
            return FontGlow.BCE(y_logits, y_onehot.float())

    @staticmethod
    def loss_class(c_logits, c):
        if c_logits is None:
            return 0
        else:
            return FontGlow.CE(c_logits, c.long())
